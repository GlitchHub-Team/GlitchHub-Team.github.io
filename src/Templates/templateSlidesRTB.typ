...
#let cover-slide(
  ...
  align(center + horizon)[
    ...
    #align(bottom)[#text(size: 2.5em, weight: "bold")[#title]]
  ...
  set page(
    ...
    footer: context {
      if counter(page).get().first() > 1 {
        ...
        place(rect(width: 100%, height: 4pt, fill: gray.lighten(80%), radius: 2pt,))

        place(rect(width: progress, height: 4pt, fill: primary, radius: 2pt,),)

        v(1.8em)

        grid(
          columns: (1fr, 1fr), align: (left, right),
          [GlitchHub Team], [#current / #total],
        )
      }
    },
  )
    ...
    footer: context {
      if counter(page).get().first() > 1 {
        ...
        rect(
          width: 100%,
          height: 3pt,
          fill: gradient.linear(primary.lighten(70%), primary.lighten(70%), primary, primary, angle: 0deg,),
        )
        ...
        grid(
          ...
          [GlitchHub Team], [#title], [*#current / #total*],
        )
      }
    },
  )
