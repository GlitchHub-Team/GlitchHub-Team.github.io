...
La riunione è stata organizzata con lo scopo di discutere alcune domande e dubbi sorti ai componenti del gruppo riguardo il capitolato C7:  _ Sistema di acquisizione dati da sensori_. \
L'incontro è stato presenziato dai rappresentanti *Alessandro Dinato* e *Siria Salvalaio* in videoconferenza sulla piattaforma _Microsoft Teams_.
= Preparazione all'incontro
Il gruppo GlitchHub Team, in data *21/10/2025*, si è riunito con lo scopo far emergere i dubbi dei singoli componenti. \
...
Quest'ultimo ha effettuato la richiesta di ricevere preventivamente le domande così da prepararsi e svolgere una riunione più fluida.
...
La risposta è stata positiva, M31 è in grado di fornire l'accesso a determinati corsi sulla piattaforma *Udemy*, sebbene consigliando di usare anche strumenti moderni, come gli LLM, per l'apprendimento.
...
L'azienda ha spiegato che la loro richiesta è che i dati di diversi tenant vengano inseriti in tabelle diverse per questioni di difficoltà di separazione a livello logico.
...
M31 ha precisato che la registrazione di nuovi sensori dalla dashboard ha lo scopo di creare un'entry con cui associare i dati proveniente dal sensore fisico, non c'è nessun requisito di setup da parte della dashboard.
...
L'azienda ha chiarito che alcune tencologie tra quelle menzionate nel documento non ha senso che vengano utizzate nell'MVP per la ripida curva di apprendimento.
M31 ha, inoltre, sconsigliato di utilizzare _Apache Kafka_ e _Kubernetes_ per il motivo citato precedentemente.
...
Ma soprattutto è stato fondamentale per chiarire ogni dubbio che era sorto ai componenti del gruppo.
