...
In presenza di errori restituiti dalle API{{gloss}} (es. permessi insufficienti, timeout del server o parametri non validi), l'applicazione mostra un banner rosso nella parte superiore dell'area di lavoro:
...
- **Gateway inattivo**: il gateway che ospita il sensore è in stato `INACTIVE`.
- **Sensore sospeso**: il sensore stesso è stato interrotto tramite comando manuale.
...
- **Mancata ricezione invito/reset**: in ambiente di test, assicurarsi di monitorare il server Mailtrap{{gloss}} per intercettare le email in uscita. 
