...
== Resoconto incontri con i professori Cardin e Vardanega
Il gruppo ha riportato l'esito positivo (semaforo verde) dell'incontro con il professor Cardin, e ha riportato i dubbi espressi dal professor Vardanega riguardanti soprattutto la mancanza di ore di orologio per poter garantire la data di consegna del MVP prevista per il 27 marzo 2026. Ci è stato chiesto se la data di consegna fosse ufficiale sia per i professori sia per l'azienda, ed è stato ragionevole pensare di stabilire che la consegna per la proponente sia alcuni giorni in anticipo.

== Pianificazione per i prossimi sprint

=== Progettazione
Partire dalla logica ad alto livello (modelli C4, diagrammicdelle classi), basandosi sul PoC e sui suggerimenti di M31, per poi scendere nel dettaglio dei singoli componenti.\
È importante organizzarsi in modo efficace, dato che il gruppo è numeroso: le task devono essere assegnate in maniera ordinata e ciascun membro deve avere un ruolo ben definito. L'approccio consiste nel passare dall'architettura ad alto livello alla definizione dei singoli componenti, per poi ampliarli progressivamente.
...
Per l'MVP, utilizzare _docker_ secrets iniettati tramite _compose_. I secrets non vanno pubblicati su GitHub. Si è pensato ad una possibile condivisione interna tramite Bitwarden, con file .env locali e script di setup automatico delle credenziali.
...
=== Priorità
Concentrarsi prima sui requisiti obbligatori, aggiungendo quelli desiderabili/opzionali in modo incrementale. Focus sulla parte core (comunicazione e scambio dati); per la dashboard usare strumenti predefiniti (es. Angular Material).
