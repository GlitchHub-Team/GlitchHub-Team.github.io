...
Durante l'incontro il gruppo ha riportato l'esito degli incontri RTB con i professori, successivamente sono stati dati consigli pratici dalla proponente su come avanzare.
...
=== Dev containers
C'è stato consigliato di sviluppare un ambiente di sviluppo in container con _hot reload_ dalla cartella locale tramite _docker_ _compose_. Utile per sincronizzare versioni e ambienti, da svolgere in parallelo alla progettazione.
...
=== Gestione repository
 - Adottare un approccio _multi-repository_ con _Dockerfile_ separati per servizio, evitando un'unica immagine troppo pesante.
 -  _Repository parent_ con _git submodule_ per orchestrare il tutto. 
 - Piccoli _compose_ separati per consentire lo sviluppo indipendente dei singoli servizi.
