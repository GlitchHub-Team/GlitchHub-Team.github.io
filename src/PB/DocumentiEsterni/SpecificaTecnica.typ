...
L'architettura del frontend è stata progettata seguendo il pattern _Model-View-ViewModel_ (MVVM), una scelta dettata dalla perfetta aderenza di questo paradigma alla filosofia strutturale di #gloss[Angular]. La natura stessa del framework, infatti, concretizza idealmente l'MVVM delegando la presentazione ai template (*View*), la mediazione dello stato alle classi dei componenti (*ViewModel*) e la business logic ai servizi iniettabili (*Model*). Sfruttando il motore di _data binding_ e le primitive _reattive_ di Angular, è stato possibile separare nettamente la logica applicativa dall'interfaccia utente: la visualizzazione si sincronizza automaticamente con le mutazioni dello stato, restituendo un codice altamente modulare, testabile e manutenibile.

Di seguito viene descritta l'organizzazione dei tre strati principali del pattern MVVM e la loro implementazione all'interno del progetto.

Il *Model* rappresenta il cuore logico e il detentore dello stato dell'applicazione. È implementato attraverso servizi di _dominio_ che gestiscono lo stato applicativo internamente tramite _Signals_ privati, esponendolo all'esterno esclusivamente in *sola lettura* (`asReadonly`). Il suo ruolo principale è l'orchestrazione della business logic, mantenendosi totalmente agnostico rispetto all'interfaccia utente. Un pilastro di questo strato è l'integrazione del *Dependency Inversion Principle*: il *Model* non dipende da implementazioni concrete per il recupero dei dati, ma inietta classi _astratte_ che definiscono i contratti di comunicazione. Questo permette al *Model* di ignorare i dettagli del backend o dei protocolli di rete, delegando l'effettivo recupero dei dati ai servizi _infrastrutturali_. Così facendo, la logica di business rimane protetta, testabile in totale isolamento e altamente estensibile.

La *View* costituisce il livello di interfaccia con l'utente ed è materializzata dai _template_ HTML e dai fogli di stile incapsulati del componente. Il suo ruolo nell'architettura è rigorosamente quello di *Passive View*: non possiede alcuna intelligenza propria e ignora totalmente l'esistenza del *Model* o dei servizi di backend. Essa si limita a riflettere visivamente lo stato esposto dal *ViewModel* tramite l'interpolazione e il _property binding_, e a intercettare le interazioni dell'utente (come click o input), inoltrandole al *ViewModel* attraverso l'_event binding_.

Il *ViewModel* agisce da mediatore tra la *View* e il *Model* ed è implementato dalla classe _TypeScript_ del componente. Esso inietta privatamente i servizi del *Model* per potervi accedere, ma si astiene dall'esporli direttamente al _template_ per preservare il principio di incapsulamento e la rigorosa separazione delle responsabilità. Per rendere i dati fruibili dalla *View* mantenendo intatta la catena della reattività, il *ViewModel* espone le *reference* ai Signals del *Model* tramite proprietà locali protette in sola lettura (`protected readonly`), oppure ne deriva di nuovi tramite _Signals computati_ (`computed`) qualora i dati grezzi necessitino di trasformazioni prima della visualizzazione. Questo strato garantisce che la *View* non debba mai farsi carico della complessità del dominio, permettendo al contempo di modificare l'interfaccia utente senza impattare minimamente sulla logica di business sottostante.
...
La presente sezione illustra nel dettaglio il #gloss[Code Diagram] del frontend #gloss[Angular], esplorando l'implementazione dell'interfaccia utente del sistema.
...
====== UI Components <angular-login-components>
...
- *`logoutUserUseCase LogoutUserUseCase`*: Riferimento all'_inbound port_ per  invalidare l'autenticazione di un utente
...
- *`LogoutUserCommand`*: Comando usato per ottenere tutti i gateway, contiene i seguenti campi:
  ...
  - `UserId`: UUID dell'utente che ha richiesto il logout, usato per invalidare l'autenticazione l'utente dal sistema
...
- *`can_impersonate BOOLEAN NOT NULL`*: `true` se il tenant è impersonificabile, `false` altrimenti.
...
In questa sezione è riportato lo stato della copertura dei requisiti descritti nel documento di #link("https://glitchhub-team.github.io/pdf/PB/DocumentiEsterni/AnalisiDeiRequisiti.pdf")[Analisi dei Requisiti].
...
== Tabelle riassuntive <tabelle-riass>
...
#tabella-paginata(
  table(
    columns: 5,
    table.header([*Tipologia*], [*RF*], [*RNF*], [*RD*], [*TOTALE*]),
  ...
  [
    Tabella rappresentante il numero di requisiti soddisfatti raggruppati per urgenza e tipologia di requisito
  ],
)
=== Percentuali requisiti soddisfatti
#tabella-paginata(
  table(
    columns: 5,
    table.header([*Tipologia*], [*RF*], [*RNF*], [*RD*], [*TOTALE*]),
    [*Obb*],
    [#perc(rf-obb, rf-obb-tot)],
    [#perc(rnf-obb, rnf-obb-tot)],
    [#perc(rd-obb, rd-obb-tot)],
    [*#perc(obb-tot-s, obb-tot)*],

    [*Des*],
    [#perc(rf-des, rf-des-tot)],
    [#perc(rnf-des, rnf-des-tot)],
    [#perc(rd-des, rd-des-tot)],
    [*#perc(des-tot-s, des-tot)*],

    [*Opt*],
    [#perc(rf-opt, rf-opt-tot)],
    [#perc(rnf-opt, rnf-opt-tot)],
    [#perc(rd-opt, rd-opt-tot)],
    [*#perc(opt-tot-s, opt-tot)*],

    [*Totale*],
    [#perc(rf-tot-s, rf-tot)],
    [#perc(rnf-tot-s, rnf-tot)],
    [#perc(rd-tot-s, rd-tot)],
    [*#perc(tot-s, tot)*],
  ),
  [Tabella rappresentante le percentuali di requisiti soddisfatti raggruppati per urgenza e tipologia di requisito],
)
== Grafici riassuntivi <grafici-riass>

=== Grafico requisiti funzionali obbligatori soddisfatti e non
#let graficoReqFunzObb = (
  ("Requisiti funzionali obbligatori soddisfatti", rf-obb),
  ("Requisiti funzionali obbligatori non soddisfatti", rf-obb-tot - rf-obb),
)
#graficoTorta("Grafico a torta rappresentante i requisiti funzionali obbligatori soddisfatti e non", graficoReqFunzObb)

=== Grafico requisiti funzionali desiderabili soddisfatti e non
#let graficoReqFunzDes = (
  ("Requisiti funzionali desiderabili soddisfatti", rf-des),
  ("Requisiti funzionali desiderabili non soddisfatti", rf-des-tot - rf-des),
)
#graficoTorta("Grafico a torta rappresentante i requisiti funzionali desiderabili soddisfatti e non", graficoReqFunzDes)

=== Grafico requisiti funzionali opzionali soddisfatti e non
#let graficoReqFunzOpt = (
  ("Requisiti funzionali opzionali soddisfatti", rf-opt),
  ("Requisiti funzionali opzionali non soddisfatti", rf-opt-tot - rf-opt),
)
#graficoTorta("Grafico a torta rappresentante i requisiti funzionali opzionali soddisfatti e non", graficoReqFunzOpt)

=== Grafico requisiti funzionali soddisfatti e non
#let graficoReqFunzTot = (
  ("Requisiti funzionali soddisfatti", rf-tot-s),
  ("Requisiti funzionali non soddisfatti", rf-tot - rf-tot-s),
)
#graficoTorta("Grafico a torta rappresentante i requisiti funzionali soddisfatti e non", graficoReqFunzTot)
