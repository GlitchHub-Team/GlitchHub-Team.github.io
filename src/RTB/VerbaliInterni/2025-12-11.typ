...
== Allineamento sulla scrittura degli use cases
Durante la stesura degli use cases (da qui in poi "UC"), sono sorti dei dubbi relativi alla definizione di "#gloss[Attore]" e di "#gloss[Sistema]" da utilizzare nella loro scrittura. Questi dubbi sono stati ulteriormente esacerbati dalle risposte contrastanti ottenute dal prof. Vardanega e l'azienda #gloss[proponente].

Il dubbio principale del gruppo risiedeva nel cosa considerare come *#gloss[sistema]*, poiché il #gloss[capitolato] del #gloss[proponente] (vd. #link("#gloss[https]://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C7.pdf")[Capitolato C7]) richiede lo sviluppo di due parti comunicanti separate, ovvero l'infrastruttura #gloss[Cloud] e il simulatore di #gloss[Gateway]. Il gruppo inizialmente non era d'accordo se considerare entrambe le parti come un #gloss[sistema] unico o considerare ciascuna parte come un #gloss[sistema] a sé stante negli UC relativi.

Dopo aver discusso, i membri del gruppo hanno deciso di:
- Considerare il *#gloss[sistema]* di uno UC come una *parte* del prodotto che interagisce con i relativi *attori*. Questo implica che il simulatore di #gloss[Gateway] e l'infrastruttura #gloss[Cloud] sono considerati due *sistemi* diversi negli UC relativi.

- Considerare un *#gloss[attore]* come una qualunque entità esterna al *#gloss[sistema]* preso in considerazione, che agisce sul suo stato (ad esempio, modificandolo o anche solo visualizzandolo). Data la definizione di *#gloss[sistema]* descritta sopra, questo implica che il #gloss[Gateway] può essere un *#gloss[attore]* nei confronti del #gloss[Cloud] e viceversa, ove questo sia appropriato.
...
== Necessità di automatizzare la creazione delle #gloss[GitHub Issues]
Durante la riunione, è sorta la necessità da parte del gruppo di rendere più veloce la creazione delle #gloss[Github Issues] associate alle task da compiere. È stato proposto di utilizzare *#gloss[Issue Form]* e delle #gloss[GitHub Actions] ad hoc per compilare automaticamente i campi ridondanti delle issues.

== Assegnazione delle prossime task basate sulle decisioni prese
Il gruppo ha deciso di proseguire con la redazione degli UC, dividendosi per #gloss[attore] principale, in modo analogo a come descritto nel #link("#gloss[https]://glitchhub-team.#gloss[github].io/pdf/#gloss[RTB]/VerbaliInterni/2025-12-03")[*verbale interno del 3 dicembre 2025*].
...
#table(
  ...
  [Sviluppo UC relativi al *#gloss[Super Admin]*], [Michele Dioli, Alessandro Dinato], [#issue(97)],

  [Sviluppo UC relativi al *#gloss[Tenant Admin]*], [Riccardo Graziani, Elia Ernesto Stellin], [#issue(98)],

  [Sviluppo UC relativi al *#gloss[Gateway]*], [Jaume Bernardi, Siria Salvalaio], [#issue(99)],
  ...
  [Creazione di *#gloss[Issue Form]* e automazione per Issue], [Elia Ernesto Stellin], [#issue(104)],
)
