...
#let lista-test = (
  ...
  [Non implementato],
  [#ts()],
  [Verificare che il gateway simulato, dopo aver eseguito il decommissioning, rimuova l'associazione con il tenant e interrompa l'invio dei dati IoT al Cloud],
  [#rf()],
  [Non implementato],
  [#ts()],
  ...
  // [#rf()],
  // [Non implementato],
  //TODO: ci sono troppi test di sistema poco atomici conseguenti da requisiti non atomici
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway sospeso il comando di riattivazione di un Sensore simulato sospeso specifico e verificare che, in seguito alla ricezione del comando il gateway, ricomincia ad inviare dati IoT crittografati relativi a tale Sensore],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway il comando di sospensione di un Sensore simulato non sospeso specifico e verificare che, in seguito alla ricezione del comando il gateway, interrompe l'invio di dati IoT crittografati relativi a tale Sensore],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare, al gateway sospeso, il comando di riattivazione e verificare che, in seguito alla ricezione del comando, il gateway ricomincia l'invio di tutti i dati IoT crittografati],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway il comando di sospensione e verificare che, in seguito alla ricezione del comando, il gateway interrompe l'invio di tutti i dati IoT crittografati],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway il comando di riavvio e verificare che,in seguito alla ricezione del comando il gateway, si spegne e riaccende, mantenendo le configurazioni e i dati salvati localmente],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway il comando di modifica della frequenza di invio dati per uno specifico tipo di sensore e verificare che, in seguito alla ricezione di tale comando il gateway deve aggiornare la configurazione della frequenza di invio dati per i dati inviati al Cloud relativi a tutti i sensori di tale tipologia, così da inviare un dato ogni intervallo di tempo specificato nel comando.],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway il comando di decommissioning e verificare che, in seguito alla ricezione del comando il gateway, cancella l'associazione al tenant ed esegue un reset. Dopo aver eseguito il decommissioning il gateway deve ammettere un nuovo commissioning],
  [#rf()],
  [Non implementato],
  [#ts()],
  [Verificare che il *Cloud* possa inviare al gateway il comando di reset e verificare che, in seguito alla ricezione del comando il gateway, cancella tutte le configurazioni e i dati salvati localmente],
  [#rf()],
  [Non implementato],
  [#ts()],
  ...
  [Non implementato],
  [#ts()],
  [Verificare che il *Sistema*, durante il decommissioning, disassoci il Gateway dal tenant],
  [#rf()],
  [Non implementato],
  [#ts()],
