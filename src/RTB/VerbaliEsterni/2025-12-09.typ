...
Eventuali scenari di offuscazione dei dati rientrano in accordi contrattuali specifici fra cliente e M31, e non sono parte del #gloss[progetto didattico].
...
Il super-admin può vedere gli utenti creati dai #gloss[tenant], ma eventuali limitazioni dipendono dai permessi contrattuali concessi per l'#gloss[impersonificazione].

=== Configurazione del gateway
Il super-admin deve poter:
- resettare la configurazione del #gloss[gateway] allo stato iniziale;
- modificare parametri operativi come la frequenza di invio dei dati;
- creare, rimuovere o modificare un'associazione di un #gloss[gateway] a un #gloss[tenant].

La configurazione dovrebbe essere:
- *estensibile*, per supportare futuri sensori o comportamenti personalizzati;
- espressa eventualmente tramite un *JSON* interpretato dal #gloss[gateway], che definisce il comportamento desiderato (es. frequenza di invio dati).

=== Provisioning e sicurezza
Per quanto riguarda il provisioning sicuro, il #gloss[proponente] ha indicato che:
- si può assumere che un #gloss[gateway] reale generi una coppia di chiavi pubblica/privata al primo avvio;
- il #gloss[gateway] stabilisce una comunicazione sicura con il #gloss[cloud] per scambiare tali chiavi;
- i certificati di autenticità sono installati da M31 nel #gloss[cloud] così da riconoscere i #gloss[gateway] autentici.

Il super-admin deve poter effettuare il *provisioning iniziale* e anche la *rimozione dell'associazione* di un #gloss[gateway].
...
#table(
  ...
  [Redazione use cases #gloss[Super Admin]], [Michele Dioli, Alessandro Dinato], [#link("#gloss[https]://#gloss[github].com/GlitchHub-Team/GlitchHub-Team.#gloss[github].io/issues/97")[*\#97*]],
  [Redazione use cases #gloss[Tenant Admin]], [Elia Ernesto Stellin, Riccardo Graziani], [#link("#gloss[https]://#gloss[github].com/GlitchHub-Team/GlitchHub-Team.#gloss[github].io/issues/98")[*\#98*]],
  [Redazione use cases #gloss[Gateway]], [Alessandro Dinato, Jaume Bernardi, Siria Salvalaio], [#link("#gloss[https]://#gloss[github].com/GlitchHub-Team/GlitchHub-Team.#gloss[github].io/issues/99")[*\#99*]],
)
